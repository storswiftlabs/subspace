...
use subspace_farmer::cluster::node_client::ClusterNodeClient;
...
/// Interval between farmer self-identification notification messages
pub(super) const FARMER_IDENTIFICATION_NOTIFICATION_INTERVAL: Duration = Duration::from_secs(60);
    ...
    PosTable: Table,
{
    ...
    let plotter = Arc::new(ClusterPlotter::new(nats_client.clone()));
    ...
    let total_and_plotted_sectors = farms
        ...
        .map(|(farm_index, farm)| async move {
            ...
            let plotted_sectors_count = farm.plotted_sectors_count().await.map_err(|error| {
                anyhow!("Failed to get plotted sectors count from from index {farm_index}: {error}")
            })?;
